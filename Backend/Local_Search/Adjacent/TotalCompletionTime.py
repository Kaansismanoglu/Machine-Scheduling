...
def calculate_total_completion_time(job_sequence, Processing_Times, num_machines):
    ...
    # Calculate total completion time by summing the completion times for each job on each machine
    for machine in range(num_machines):
...
def Total_Completion_Time_Adjacent(Processing_Times, job_sequence, num_machines, iterations=None, runtime=None):
    start_iteration_time = time.time()  # Start time tracking
    current_total_completion_time = calculate_total_completion_time(job_sequence, Processing_Times, num_machines)  # Calculate initial total completion time
    
    output_lines = []  # List to store output lines for the text file
    output_lines.append(f"Initial Solution: {job_sequence}, Total Completion Time: {current_total_completion_time}\n")
    output_lines.append("-----------------------------\n")
    ...
    while True:  # Continue indefinitely
        ...
        new_total_completion_time = calculate_total_completion_time(new_sequence, Processing_Times, num_machines)  # Calculate total completion time after swap
        
        if new_total_completion_time <= current_total_completion_time:  # Check if the new total completion time is better
            current_total_completion_time = new_total_completion_time  # Update current total completion time
            job_sequence = new_sequence.copy()  # Update current sequence
            best_solution = new_sequence.copy()  # Update best solution
            output_lines.append(f"Iteration: {iteration_count}, New Best Solution: {new_sequence}, Total Completion Time: {new_total_completion_time}\n")
            output_lines.append("------------Updated-------------\n")
            
            iteration_count += 1
        else:
            output_lines.append(f"Iteration: {iteration_count}, Worse Solution: {new_sequence}, Total Completion Time: {new_total_completion_time}\n")
            output_lines.append("VS.\n")
            output_lines.append(f"Current Best Solution: {best_solution}, Total Completion Time: {current_total_completion_time}\n")
    ...
    plt.text(completion_times[-1][-1] / 2, num_machines + 0.5, f'Total Completion Time: {current_total_completion_time}', 
             ha='center', va='center', fontsize=12, color='red')  # Add total completion time value to the chart
    ...
    with open("total_completion_time_output.txt", "w") as f:
