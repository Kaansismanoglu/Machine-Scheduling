...
def calculate_total_weighted_tardiness(job_sequence, Processing_Times, Due_Date, Weights, num_machines):
    ...
    for job_index in range(num_jobs):
        ...
        weight = Weights.iloc[job_sequence[job_index] - 1]  # Get weight for the job
        
        tardiness = max(0, completion_time - due_date)  # Calculate tardiness
        total_weighted_tardiness += weight * tardiness  # Calculate weighted tardiness
...
def Total_Weighted_Tardiness_Adjacent(Processing_Times, job_sequence, Due_Date, Weights, num_machines, iterations=None, runtime=None):
    start_iteration_time = time.time()  # Start time tracking
    current_total_weighted_tardiness = calculate_total_weighted_tardiness(job_sequence, Processing_Times, Due_Date, Weights, num_machines)  # Calculate initial total weighted tardiness
    
    output_lines = []  # List to store output lines for the text file
    output_lines.append(f"Initial Solution: {job_sequence}, Total Weighted Tardiness: {current_total_weighted_tardiness}\n")
    output_lines.append("-----------------------------\n")
    ...
    while True:  # Continue indefinitely
        ...
        new_total_weighted_tardiness = calculate_total_weighted_tardiness(new_sequence, Processing_Times, Due_Date, Weights, num_machines)  # Calculate total weighted tardiness after swap
        
        if new_total_weighted_tardiness <= current_total_weighted_tardiness:  # Check if the new total weighted tardiness is better
            current_total_weighted_tardiness = new_total_weighted_tardiness  # Update current total weighted tardiness
            job_sequence = new_sequence.copy()  # Update current sequence
            best_solution = new_sequence.copy()  # Update best solution
            output_lines.append(f"Iteration: {iteration_count}, New Best Solution: {new_sequence}, Total Weighted Tardiness: {new_total_weighted_tardiness}\n")
            output_lines.append("------------Updated-------------\n")
            
            iteration_count += 1
        else:
            output_lines.append(f"Iteration: {iteration_count}, Worse Solution: {new_sequence}, Total Weighted Tardiness: {new_total_weighted_tardiness}\n")
            output_lines.append("VS.\n")
            output_lines.append(f"Current Best Solution: {best_solution}, Total Weighted Tardiness: {current_total_weighted_tardiness}\n")
    ...
    plt.text(completion_times[-1][-1] / 2, num_machines + 0.5, f'Total Weighted Tardiness: {current_total_weighted_tardiness}', 
             ha='center', va='center', fontsize=12, color='red')  # Add total weighted tardiness value to the chart
    ...
    with open("total_weighted_tardiness_output.txt", "w") as f:
