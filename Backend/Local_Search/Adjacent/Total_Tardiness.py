...
def calculate_total_tardiness(job_sequence, Processing_Times, Due_Date, num_machines):
...
def Total_Tardiness_Adjacent(Processing_Times, job_sequence, Due_Date, num_machines, iterations=None, runtime=None):
    start_iteration_time = time.time()  # Start time tracking
    current_total_tardiness = calculate_total_tardiness(job_sequence, Processing_Times, Due_Date, num_machines)  # Calculate initial total tardiness
    
    output_lines = []  # List to store output lines for the text file
    output_lines.append(f"Initial Solution: {job_sequence}, Total Tardiness: {current_total_tardiness}\n")
    output_lines.append("-----------------------------\n")
    ...
    while True:  # Continue indefinitely
        ...
        new_total_tardiness = calculate_total_tardiness(new_sequence, Processing_Times, Due_Date, num_machines)  # Calculate total tardiness after swap
        
        if new_total_tardiness <= current_total_tardiness:  # Check if the new total tardiness is better
            current_total_tardiness = new_total_tardiness  # Update current total tardiness
            job_sequence = new_sequence.copy()  # Update current sequence
            best_solution = new_sequence.copy()  # Update best solution
            output_lines.append(f"Iteration: {iteration_count}, New Best Solution: {new_sequence}, Total Tardiness: {new_total_tardiness}\n")
            output_lines.append("------------Updated-------------\n")
            
            iteration_count += 1
        else:
            output_lines.append(f"Iteration: {iteration_count}, Worse Solution: {new_sequence}, Total Tardiness: {new_total_tardiness}\n")
            output_lines.append("VS.\n")
            output_lines.append(f"Current Best Solution: {best_solution}, Total Tardiness: {current_total_tardiness}\n")
    ...
    plt.text(completion_times[-1][-1] / 2, num_machines + 0.5, f'Total Tardiness: {current_total_tardiness}', 
             ha='center', va='center', fontsize=12, color='red')  # Add total tardiness value to the chart
    ...
    with open("total_tardiness_output.txt", "w") as f:
